...
function firstNotes(changelog, version) {
  const escaped = version.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); const date = "\\([0-9]{4}-[0-9]{2}-[0-9]{2}\\)"; const heading = new RegExp(`^## (?:${escaped} ${date}|\\[${escaped}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-updater-support/compare/v[^)\\s]+\\.\\.\\.v${escaped}\\) ${date})\\n`, "m");
...
export function candidateIdentity(contents, candidateSha) {
  ...
  if (composer?.name !== "ran/updater-support" || composer?.type !== "library" || Object.hasOwn(composer, "version")) refuse("composer_identity_invalid", "Composer identity is not the unversioned target library");
  return { candidateSha, packageName: composer.name, tag: `v${version}`, version, notes: firstNotes(contents.changelog, version) };
}
...
export function verifyReleaseDelta(parent, candidate) {
  ...
  if (candidate.composer !== parent.composer) refuse("release_content_drift", "Composer metadata may not change in a release PR");
  if (candidate.manifest !== parent.manifest.replace(JSON.stringify(before), JSON.stringify(after))) refuse("release_content_drift", "manifest may change only its version token");
  ...
  const parentHeading = before === UNRELEASED
    ? /^## \[Unreleased\]\n/m
    : new RegExp(`^## (?:${escapedBefore} ${date}|\\[${escapedBefore}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-updater-support/compare/v[^)\\s]+\\.\\.\\.v${escapedBefore}\\) ${date})\\n`, "m");
  const candidateHeading = before === UNRELEASED
    ? new RegExp(`^## ${escapedAfter} ${date}\\n`, "m")
    : new RegExp(`^## \\[${escapedAfter}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-updater-support/compare/v${escapedBefore}\\.\\.\\.v${escapedAfter}\\) ${date}\\n`, "m");
