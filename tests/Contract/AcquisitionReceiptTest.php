...
final class AcquisitionReceiptTest extends TestCase {
	...
	public function testPublicReadyBlockedAndClonePackagesCannotMintAndFlagsAreNotInputs(): void {
		...
		try {
			AcquisitionReceipt::issue( $state, $descriptor, $validator, $package, 10 );
			self::fail( 'Package proof minted twice.' );
		} catch ( InvalidArgumentException ) {
			self::addToAssertionCount( 1 ); }
	}
	...
	public function testReceiptIsBoundToClaimIncarnationAndExpiry(): void {
		...
		try {
			...
			self::fail( 'Expired lease minted receipt.' );
		} catch ( InvalidArgumentException ) {
			self::addToAssertionCount( 1 ); }
	}
	...
	private function archive(): string {
		...
		self::assertTrue( $zip->addFromString( 'x/x.php', "<?php\n/*\nPlugin Name: X\nVersion: 1.0.0\nUpdate URI: https://example.com/owner/repo\n*/" ) );
		$zip->close();
		return $path; }
	...
	private function policy(): array {
		return array(
			'archive_root'               => 'x',
			'configuration_update_uri'   => 'https://example.com/owner/repo',
			'header_file'                => 'x.php',
			'installed_package_identity' => 'x/x.php',
			'maximum_artifact_bytes'     => 52_428_800,
			'metadata_name'              => 'X',
			'offer_update_uri'           => 'https://example.com/owner/repo',
			'php_runtime_version'        => '8.2',
			'provider_code'              => 'github',
			'repository_identity'        => 'repo:1',
			'repository_locator'         => 'owner/repo',
			'staged_package_update_uri'  => 'https://example.com/owner/repo',
			'target_type'                => 'plugin',
			'theme_template'             => '',
