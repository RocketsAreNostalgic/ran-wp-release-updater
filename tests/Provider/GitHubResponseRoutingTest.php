...
final class GitHubResponseRoutingTest extends TestCase {
	...
	protected function tearDown(): void {
		foreach ( $GLOBALS['ran_github_temp_paths'] as $path ) if ( is_string( $path ) && ( is_file( $path ) || is_link( $path ) ) ) @unlink( $path );
	}
	public function testListingRateFactsStopOnTheFirstAndSecondPage(): void {
		...
		self::assertSame( array( 'limited' => true, 'remaining' => null, 'reset_at' => null, 'retry_after' => 12 ), $first['rate_limit'] );
		self::assertCount( 1, $GLOBALS['ran_github_requests'] );
		self::assertSame( array(), $GLOBALS['ran_github_responses'] );

		$page = array();
		for ( $id = 1; $id <= 20; ++$id ) { $release = $this->release( $id, 'v1.0.' . $id ); $release['draft'] = true; $page[] = $release; }
		$GLOBALS['ran_github_requests'] = array();
	...
	private function service(): GitHubReleaseService {
		return new GitHubReleaseService( array( 'canonical_repository_locator' => 'owner/repository', 'canonical_update_uri' => 'https://github.com/owner/repository', 'maximum_artifact_bytes' => 52428800, 'php_runtime_version' => '8.2', 'release_channel' => 'stable', 'stable_repository_identity' => '99', 'target_type' => 'plugin', 'wordpress_runtime_version' => '6.8' ) );
	}
	/** @return array<string,mixed> */
	private function release( int $id, string $tag ): array {
		return array( 'assets' => array( array( 'digest' => 'sha256:' . hash( 'sha256', 'zip-data' ), 'id' => 8, 'name' => 'repository.zip', 'size' => 8, 'state' => 'uploaded' ) ), 'draft' => false, 'html_url' => 'https://github.com/owner/repository/releases/tag/' . $tag, 'id' => $id, 'immutable' => true, 'prerelease' => false, 'published_at' => '2026-08-22T10:00:00Z', 'tag_name' => $tag );
	}
	/** @return array<string,mixed> */
	private function response( int $code, mixed $json, array $headers = array(), ?string $file = null ): array {
		$response = array( 'body' => null === $json ? '' : json_encode( $json, JSON_THROW_ON_ERROR ), 'headers' => $headers, 'response' => array( 'code' => $code ) );
		if ( null !== $file ) $response['file'] = $file;
		return $response;
	}
	private function assertFailure( callable $call, string $code, int $requests, ?int $retry = null, string $cleanup = 'not_applicable' ): void {
		try { $call(); self::fail( 'Expected a typed provider failure.' ); }
		catch ( ReleaseFailure $failure ) { self::assertSame( $code, $failure->releaseCode ); self::assertSame( $retry, $failure->retryAfter ); self::assertSame( $cleanup, $failure->cleanupStatus ); }
	...
	private function assertTemporaryPathsAbsent(): void {
		...
		foreach ( $GLOBALS['ran_github_temp_paths'] as $path ) self::assertFileDoesNotExist( $path );
	}
}
