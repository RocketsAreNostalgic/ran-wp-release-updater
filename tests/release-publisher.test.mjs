...
function pull() {
	return {
		...
		head: {
			ref: 'release-please--branches--main--components--ran/wp-release-updater',
			sha: 'd'.repeat(40),
			repo: { id: ID, full_name: REPOSITORY },
		},
...
test('release delta permits only manifest/runtime-copy version and a changelog prepend', () => {
	...
	assert.deepEqual(verifyReleaseDelta(candidate, later), {
	...
	refusal('release_content_drift', () =>
		verifyReleaseDelta(parent, {
			...candidate,
			runtimeCopy: candidate.runtimeCopy.replace(
				REVISION,
				'c'.repeat(64)
			),
		})
...
test('only exact green CI normal merge and changed paths can publish', () => {
	...
	const input = {
		...
		repositoryId: ID,
		immutableAcknowledgement: String(ID),
		tagRef: null,
		release: null,
		commit: {
			sha: SHA,
	...
	assert.deepEqual(decidePublication(input), {
...
test('only the exact historical runtime-copy partial parent is bootstrap metadata', () => {
	const legacy = {
		...
		runtimeCopy: {
			mode: '100644',
			type: 'blob',
			sha: 'ec1170ddb999e78bb2f30814e3760c468763e399',
		},
		changelog: null,
	};
	assert.equal(
	...
	for (const [sha, entries] of [
		['a'.repeat(40), legacy],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
			{
				...legacy,
				runtimeCopy: { ...legacy.runtimeCopy, sha: 'b'.repeat(40) },
			},
		],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
			{
				...legacy,
				runtimeCopy: { ...legacy.runtimeCopy, mode: '100755' },
			},
		],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
			{ ...legacy, manifest: otherBlob },
		],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
			{ ...legacy, changelog: otherBlob },
		],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
			{ manifest: otherBlob, runtimeCopy: null, changelog: null },
		],
		[
			'ec9e1058939b3d699fb49eb700dded1a2caddb19',
...
test('immutable release readback rejects mutable or asset-bearing releases', () => {
	...
	refusal('release_state_conflict', () =>
		verifyPublishedState(tag, { ...release, immutable: false }, identity)
	);
});
	...
	return createHash('sha256')
		.update(
			Object.entries(files)
				...
				.map(
					([file, value]) =>
						`${file}\0${createHash('sha256').update(value).digest('hex')}\n`
				)
				.join('')
		)
		.digest('hex');
...
function publisherFixture() {
	const root = mkdtempSync(join(tmpdir(), 'wp-release-publisher-'));
	...
	const write = (version, changelog) => {
		for (const [file, value] of Object.entries(source)) {
		...
		writeFileSync(
			join(root, 'runtime-copy.json'),
			JSON.stringify({
				package_revision: revision(source),
				package_version: version,
			})
		);
		writeFileSync(
			join(root, 'composer.json'),
			JSON.stringify({ name: 'ran/wp-release-updater', type: 'library' })
		);
	...
	return { root, base, head, candidate, tree, eventPath };
}
function transport(fixture, options = {}) {
	...
	const fetch = async (url, init = {}) => {
		...
		if (
			value.pathname.includes('/labels/autorelease%3A%20pending') &&
			method === 'DELETE'
		) {
			state.labels = ['autorelease: tagged'];
			return response(null, 204);
...
test('runPublisher uses a temporary Git merge and exact mocked publication sequence', async (context) => {
	...
	assert.equal(
		mocked.calls.filter(
			(call) => call.method === 'POST' && call.path.endsWith('/releases')
		).length,
		1
	);
	const release = mocked.calls.findIndex(
		(call) => call.method === 'POST' && call.path.endsWith('/releases')
	);
	const label = mocked.calls.findIndex(
		(call) => call.method !== 'GET' && call.path.includes('/labels')
	);
	assert.ok(
		mocked.calls.findIndex(
			(call, index) =>
	...
	assert.equal(
		(await runPublisher(fixture.root)).action,
		'already_published'
	);
	assert.equal(
		mocked.calls.filter(
...
test('first release setup merge has no parent metadata and remains ordinary', async (context) => {
	const fixture = publisherFixture();
	git(fixture.root, ['checkout', fixture.base]);
	git(fixture.root, [
		'rm',
		'.release-please-manifest.json',
		'runtime-copy.json',
	...
	git(fixture.root, ['commit', '-m', 'chore: pre-release setup parent']);
	const bare = git(fixture.root, ['rev-parse', 'HEAD']);
	git(fixture.root, [
		'checkout',
		'-B',
		'release-please--branches--main--components--ran/wp-release-updater',
	...
	writeFileSync(
		join(fixture.root, 'runtime-copy.json'),
		JSON.stringify({
			package_revision: revision({
				'bootstrap.php': '<?php\n',
				'runtime.php': '<?php\n',
	...
	writeFileSync(
		join(fixture.root, 'CHANGELOG.md'),
		'# Changelog\n\n## [Unreleased]\n\nBootstrap\n'
	);
	git(fixture.root, [
		'add',
	...
	git(fixture.root, ['commit', '-m', 'chore: release setup']);
	fixture.head = git(fixture.root, ['rev-parse', 'HEAD']);
	git(fixture.root, ['checkout', 'main']);
	git(fixture.root, ['reset', '--hard', bare]);
	git(fixture.root, ['merge', '--no-ff', '--no-commit', fixture.head]);
	git(fixture.root, ['commit', '-m', 'Merge release setup']);
	fixture.base = bare;
	fixture.candidate = git(fixture.root, ['rev-parse', 'HEAD']);
	...
	assert.equal(
		mocked.calls.filter((call) => call.path.includes('/git/commits/'))
			.length,
		0
	);
});

test('arbitrary runtime-copy-only partial parent rejects without writes', async (context) => {
	const fixture = publisherFixture();
	git(fixture.root, ['checkout', fixture.base]);
	git(fixture.root, ['rm', '.release-please-manifest.json', 'CHANGELOG.md']);
	git(fixture.root, ['commit', '-m', 'chore: partial parent']);
	const bare = git(fixture.root, ['rev-parse', 'HEAD']);
	git(fixture.root, [
		'checkout',
		'-B',
...
test('released metadata cannot be downgraded to unreleased or use executable blobs', () => {
	const identity = { candidateSha: SHA, version: '0.0.0' };
