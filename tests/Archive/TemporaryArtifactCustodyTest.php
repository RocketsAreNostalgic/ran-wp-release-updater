...
final class TemporaryArtifactCustodyTest extends TestCase
{
	...
	public function testInspectChecksContinuityBeforeAndAfterReader(): void
	{
		...
		$artifact->inspect(function (string $path): string {
	...
	public function testInspectPreservesCallerExceptionAndClearsBusyState(): void
	{
		...
		try {
			$artifact->inspect(static function () use ($expected): never {
				throw $expected;
			});
	...
	{
		$artifact = $this->artifact('original');
		$codes = $artifact->inspect(function () use ($artifact): array {
			$codes = array();
			foreach (array(
	...
	{
		$live = true;
		$artifact = $this->artifact('original', function () use (&$live): ?string {
			return $live ? null : 'runtime_revoked';
		});

		$this->expectException(\RuntimeException::class);
		$this->expectExceptionCode(1002);
		$artifact->inspect(function () use (&$live): string {
			$live = false;
			return 'must not escape';
		});
	}
	...
	private function artifact(string $contents, ?callable $livenessGuard = null): TemporaryArtifact
	{
		...
		return new TemporaryArtifact($path, hash_file('sha256', $path), array(
			'dev' => (int) $stat['dev'], 'ino' => (int) $stat['ino'], 'mode' => (int) $stat['mode'],
			'nlink' => (int) $stat['nlink'], 'uid' => (int) $stat['uid'], 'gid' => (int) $stat['gid'],
			'size' => (int) $stat['size'], 'mtime' => (int) $stat['mtime'], 'ctime' => (int) $stat['ctime'],
		), $livenessGuard);
	}
