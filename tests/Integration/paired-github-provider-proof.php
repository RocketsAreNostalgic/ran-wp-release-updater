...
try {
	...
	final class PairedOldTransport implements \RAN\WPGitHubReleaseUpdater\V1\Http\Transport {
		...
		public function queue( int $status, mixed $body = '', array $headers = array(), ?string $file = null ): void {
			$this->queue[] = array(
				'response' => new \RAN\WPGitHubReleaseUpdater\V1\Http\Response( $status, $headers, is_string( $body ) ? $body : json_encode( $body, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES ) ),
				'file'     => $file,
			);
		}
		public function get( \RAN\WPGitHubReleaseUpdater\V1\Http\Request $request ) {
			$this->requests[] = array(
				...
				'limit'     => $request->responseSizeLimit(),
				'redirects' => $request->redirection(),
			);
