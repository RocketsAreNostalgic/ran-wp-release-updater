...
function rs_response( mixed $body, ?string $file = null ): array {
	return array(
...
function rs_assert( bool $condition, string $message ): void {
	if ( ! $condition ) {
		throw new RuntimeException( $message ); } }
function rs_activate(): void {
	foreach ( $GLOBALS['rs_hooks'] as $hook ) {
		if ( 'after_setup_theme' === $hook['hook'] ) {
			( $hook['callback'] )(); }
	} }
/** @return array{requests:int,zips:int} */
function rs_delta( int $start ): array {
...
function rs_assert_delta( string $operation, array $delta, int $zips ): void {
	rs_assert( 0 < $delta['requests'], $operation . ' made no HTTP requests.' );
	rs_assert( $zips === $delta['zips'], $operation . ' ZIP request count changed.' ); }
...
if ( str_starts_with( $scenario, 'fence-' ) ) {
	...
	if ( 'fence-acquire' === $scenario ) {
		...
		if ( 'fence-list' === $scenario ) {
			...
			rs_assert( isset( $application_owned_path ) && $root . '/prepared-release.zip' === $application_owned_path, 'Acquisition fence did not construct its application-owned destination.' );
...
rs_assert( is_file( $GLOBALS['rs_paths'][1] ), 'Acquisition artifact was not retained.' );
...
try {
	...
	rs_assert( $caller === $actual, 'Callback identity changed.' ); }
...
try {
	if ( 'happy' === $scenario ) {
		...
		rs_assert( $artifact->discard(), 'Artifact cleanup failed.' );
		rs_assert( is_file( $provisional ), 'Prepared copy missing.' ); } elseif ( 'liveness' === $scenario ) {
		try {
			$artifact->inspect(
				static function ( string $path ) use ( $provisional, $acquisition ): void {
					rs_copy_verified( $path, $provisional, $acquisition['value']['inspection'] );
