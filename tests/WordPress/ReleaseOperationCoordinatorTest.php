...
final class ReleaseOperationCoordinatorTest extends TestCase {
	...
	public function testExpiredClaimCanReplaceBindingAndFencesTheStaleWriter(): void {
		$database = new FakeOptionDatabase( 100 );
		$old      = $this->binding();
		$first    = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $old, str_repeat( 'a', 64 ), 20 );
		...
		$replacement = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $next, str_repeat( 'b', 64 ), 20 );
		...
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::verifyPersistentBindingState( $database, $first['current'], $claim )['result'] );
		$database->setTime( 142 );
		$restart = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $next, str_repeat( 'c', 64 ), 20 );
	...
	public function testExactValueCasAndLiveLeaseRejectRacesAndTakeover(): void {
		$database = new FakeOptionDatabase( 100 );
		$binding  = $this->binding();
		$first    = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $binding, str_repeat( 'a', 64 ), 20 );
		...
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::renewPersistentBindingState( $database, $first['current'], $this->claim( $first['current'] ), 20 )['result'] );
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::claimPersistentBindingState( $database, $binding, str_repeat( 'b', 64 ), 20 )['result'] );
	}
	...
	public function testEveryProviderRepositoryUriAndPolicySwitchFencesStaleState(): void {
		foreach ( array(
			...
			$database->setTime( 121 );
			self::assertSame( 'claimed', ReleaseOperationCoordinator::claimPersistentBindingState( $database, $next, str_repeat( 'b', 64 ), 20 )['result'], $key );
			self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::verifyPersistentBindingState( $database, $first['current'], $claim )['result'], $key );
		}
	}
	public function testRenewalFencesTheOldClaimAndUsesADatabaseTimeCas(): void {
		...
		$renewed = ReleaseOperationCoordinator::renewPersistentBindingState( $database, $first['current'], $this->claim( $first['current'] ), 1 );
		...
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::verifyPersistentBindingState( $database, $first['current'], $this->claim( $first['current'] ) )['result'] );
		self::assertSame( 'verified', ReleaseOperationCoordinator::verifyPersistentBindingState( $database, $renewed['current'], $this->claim( $renewed['current'] ) )['result'] );
	...
	public function testReleasePreservesTheBindingAndLetsALaterProviderSwitchClaim(): void {
		...
		$old      = $this->binding();
		$first    = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $old, str_repeat( 'a', 64 ), 20 );
		$released = ReleaseOperationCoordinator::releasePersistentBindingState( $database, $first['current'], $this->claim( $first['current'] ) );
		...
		$database->setTime( 101 );
		$next    = BindingRecord::create( array_merge( $this->facts(), array( 'provider_code' => 'gitlab' ) ) );
		$claimed = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $next, str_repeat( 'b', 64 ), 20 );
		self::assertSame( 'claimed', $claimed['result'] );
	...
	public function testReleaseRejectsExpiredAndRacingOwners(): void {
		...
		$claim    = $this->claim( $first['current'] );
		$database->setTime( 121 );
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::releasePersistentBindingState( $database, $first['current'], $claim )['result'] );
		$database = new FakeOptionDatabase( 100 );
		$first    = ReleaseOperationCoordinator::claimPersistentBindingState( $database, $binding, str_repeat( 'a', 64 ), 20 );
		$name     = 'ran_wp_release_updater_target_v1_' . BindingRecord::targetFenceKey(
			array(
		...
		self::assertSame( 'binding_fence_lost', ReleaseOperationCoordinator::releasePersistentBindingState( $database, $first['current'], $this->claim( $first['current'] ) )['result'] );
	}
