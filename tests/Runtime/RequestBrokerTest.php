...
PHP
		...
		$result = $this->probe(
			'require $data["foreign"] . "/bootstrap.php"; $foreign=$GLOBALS["ran_wp_release_updater_v1_broker"]; $registrar=require $data["copy"] . "/bootstrap.php"; $handle=$registrar->plugin("github","/missing.php","acme/example","123"); $handle->register(); echo json_encode(array("unchanged"=>$foreign===$GLOBALS["ran_wp_release_updater_v1_broker"],"protocol"=>$registrar->diagnostics()["protocol_version"],"state"=>$registrar->diagnostics()["state"],"code"=>$handle->status()["code"]));',
			array(
				'copy'    => $copy,
				'foreign' => $foreign,
			)
