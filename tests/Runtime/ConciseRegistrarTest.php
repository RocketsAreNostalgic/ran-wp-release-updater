...
PHP
		...
		$fresh = $this->probe(
			<<<'PHP'
$r=require $data['bootstrap'];$h=$r->plugin('github',$data['late'],'acme/late-plugin','567890123','stable','disabled');$h->register();$GLOBALS['ran_wp_release_updater_v1_broker']->activate(['php_version'=>PHP_VERSION,'runtime_protocol' => 5,'wordpress_version'=>'6.8.0']);echo json_encode($h->status());
PHP
		);
