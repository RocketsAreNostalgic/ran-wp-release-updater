...
PHP
		...
		self::assertSame(
			array(
				'state'       => 'inactive',
				'diagnostics' => array( array( 'code' => 'maximum_artifact_bytes_invalid' ) ),
			),
			$result['invalid_diagnostics']
		);
...
PHP
		...
		$fresh = $this->probe(
			<<<'PHP'
$r=require $data['bootstrap'];$h=$r->plugin('github',$data['late'],'acme/late-plugin','567890123','stable','disabled');$h->register();$GLOBALS['ran_wp_release_updater_v1_broker']->activate(['php_version'=>PHP_VERSION,'runtime_protocol' => 4,'wordpress_version'=>'6.8.0']);echo json_encode($h->status());
PHP
		);
...
PHP
	...
	public function testNativeOfferIdentityMustBeBoundedAndPresentExactlyWithTheOfferVersion(): void {
		...
		$validate = new \ReflectionMethod( $broker, 'validNativeStatus' );
		$status   = array(
		...
		self::assertTrue( $validate->invoke( $broker, $status ) );
		$status['offered_release_identity'] = str_repeat( 'x', 192 );
		self::assertFalse( $validate->invoke( $broker, $status ) );
		$status['offered_release_identity'] = 'release:42';
		$status['offered_version']          = null;
		self::assertFalse( $validate->invoke( $broker, $status ) );
	}
...
PHP
		...
		foreach ( $result as $index => $row ) {
			...
			self::assertSame( $codes[ $index ], $row[2]['code'] );
			self::assertFalse( $row[2]['declaration_accepted'] );
			self::assertFalse( $row[2]['hooks_registered'] );
			self::assertNull( $row[2]['native'] );
	...
	private function probe( string $body, array $extra = array() ): array {
		...
		$data   = array_merge(
			array(
				...
				'theme'     => $this->root . '/theme/style.css',
				'manager'   => $this->root . '/manager-theme/style.css',
				'forced'    => $this->root . '/forced-plugin/main.php',
				'late'      => $this->root . '/late-plugin/main.php',
				'canonical' => $this->root . '/canonical/plugin.php',
				'mismatch'  => $this->root . '/mismatch/plugin.php',
				'root'      => $this->root,
			),
			$extra
		);
