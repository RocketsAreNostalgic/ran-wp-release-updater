...
PHP );
	...
	private function probe( string $body, array $extra = array() ): array
	{
		...
		$file = $root . '/probe.php';
		$data = array_merge( array( 'bootstrap' => dirname( __DIR__, 2 ) . '/bootstrap.php' ), $extra );
		$prefix = '<?php require ' . var_export( dirname( __DIR__, 5 ) . '/wp-includes/plugin.php', true ) . '; $GLOBALS["wp_version"]="6.8.0"; $data=' . var_export( $data, true ) . ';';
