...
final class ReleaseSourceSchemaTest extends TestCase
{
	...
	{
		$result = $this->projection('list', $this->envelope('releases_listed', array(
			'candidates' => array(),
			'conditional' => array('etag' => null, 'last_modified' => null),
			'not_modified' => false,
			'rate_limit' => array('limited' => false, 'remaining' => null, 'reset_at' => null, 'retry_after' => 0),
	...
	{
		$invalid = array('ok' => false, 'code' => 'invalid_configuration', 'value' => null, 'retry_after' => null, 'cleanup_status' => 'complete');
		self::assertSame('runtime_unavailable', $this->projection('inspect', $invalid)['code']);
		$repository = array('ok' => false, 'code' => 'repository_access_unavailable', 'value' => null, 'retry_after' => null, 'cleanup_status' => 'complete');
	...
	public function testMalformedAcquisitionCleansTheRealOwnedArtifactBeforeReturn(): void
	{
		...
		foreach (array('nested' => 'runtime_unavailable', 'outer' => 'operation_failed') as $mutation => $expectedCode) {
		...
		self::assertSame(1, $result['calls']); self::assertSame($expectedCode, $result['result']['code']); self::assertSame('complete', $result['result']['cleanup_status']); self::assertFalse($result['exists']);
		}
	}
	...
	private function malformedResults(): array
	{
		...
		return array(
			...
			'changed-without-proof-cleanup' => array('acquire', array('ok' => false, 'code' => 'release_changed', 'value' => null, 'retry_after' => null, 'cleanup_status' => 'not_applicable')),
			'extra-listing-path' => array('list', $this->envelope('releases_listed', array('candidates' => array(), 'conditional' => array('etag' => null, 'last_modified' => null), 'not_modified' => false, 'rate_limit' => array('limited' => false, 'remaining' => null, 'reset_at' => null, 'retry_after' => 0), 'search_exhausted' => true, 'secret_path' => '/private'), 'not_applicable')),
	...
	private function projection(string $operation, array $result): array
	{
		...
		$script = '<?php '
			...
			. '$broker=$GLOBALS["ran_wp_release_updater_v1_broker"];$activation=$broker->activate(["php_version"=>PHP_VERSION,"runtime_protocol"=>3,"wordpress_version"=>"6.8.0"]);'
