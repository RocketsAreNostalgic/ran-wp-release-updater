...
export function manifestVersion(raw, source, permitUnreleased = false) {
	...
	try {
		...
		refuse('release_manifest_invalid', `${source} manifest is not JSON`);
	}
	const version = manifest?.['.'];
	if (
		Object.keys(manifest ?? {}).length !== 1 ||
		typeof version !== 'string' ||
		(!BETA.test(version) && !(permitUnreleased && version === UNRELEASED))
	) {
		refuse(
			'release_manifest_invalid',
			`${source} manifest must contain one canonical release version`
		);
	}
	return version;
}
...
export function runtimeCopy(raw) {
	...
	try {
		...
		refuse('runtime_copy_invalid', 'runtime-copy.json is not JSON');
	}
	if (
		!copy ||
		typeof copy !== 'object' ||
		Array.isArray(copy) ||
		!/^[a-f0-9]{64}$/.test(copy.package_revision ?? '') ||
		typeof copy.package_version !== 'string'
	) {
		refuse(
			'runtime_copy_invalid',
			'runtime-copy.json has no canonical package identity'
		);
	}
	return copy;
}
...
function firstNotes(changelog, version) {
	...
	const heading = new RegExp(
		`^## (?:${escaped} ${date}|\\[${escaped}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-release-updater/compare/v[^)\\s]+\\.\\.\\.v${escaped}\\) ${date})\\n`,
		'm'
	);
...
export function candidateIdentity(contents, candidateSha) {
	...
	try {
		...
		refuse('composer_identity_invalid', 'composer.json is not JSON');
	}
	if (
	...
	return {
		...
		tag: `v${version}`,
		version,
		notes: firstNotes(contents.changelog, version),
		packageRevision: copy.package_revision,
	};
}
...
export function verifyReleaseDelta(parent, candidate) {
	...
	if (before === UNRELEASED && after !== '0.1.0-beta.1') {
		refuse(
			...
			'first release must be 0.1.0-beta.1'
		);
	}
	...
	if (
		...
		candidate.runtimeCopy !==
			parent.runtimeCopy.replace(
				JSON.stringify(before),
				JSON.stringify(after)
			)
	) {
	...
	const parentHeading =
		before === UNRELEASED
			? /^## \[Unreleased\]\n/m
			: new RegExp(
					`^## (?:${escapedBefore} ${date}|\\[${escapedBefore}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-release-updater/compare/v[^)\\s]+\\.\\.\\.v${escapedBefore}\\) ${date})\\n`,
					'm'
				);
	const candidateHeading =
		before === UNRELEASED
			...
			: new RegExp(
					`^## \\[${escapedAfter}\\]\\(https://github\\.com/RocketsAreNostalgic/ran-wp-release-updater/compare/v${escapedBefore}\\.\\.\\.v${escapedAfter}\\) ${date}\\n`,
					'm'
				);
	const parentMatch = parentHeading.exec(parent.changelog);
	const candidateMatch = candidateHeading.exec(candidate.changelog);
	if (
		!parent.changelog.startsWith(prefix) ||
