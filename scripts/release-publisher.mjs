...
const LEGACY_BOOTSTRAP_RUNTIME_COPY_BLOB =
	'ec1170ddb999e78bb2f30814e3760c468763e399';
...
function contents(root, sha) {
	return {
		manifest: blob(root, sha, '.release-please-manifest.json'),
		runtimeCopy: blob(root, sha, 'runtime-copy.json'),
...
function parentContents(root, sha) {
	const state = classifyParentReleaseMetadata(sha, {
		manifest: treeEntry(root, sha, '.release-please-manifest.json'),
		runtimeCopy: treeEntry(root, sha, 'runtime-copy.json'),
...
export function revisionAt(root, sha) {
	const files = git(root, ['ls-tree', '-r', '--name-only', sha])
		.split('\n')
		.filter(
			(file) =>
				file === 'bootstrap.php' ||
...
function shaped(pull) {
	return (
		...
		labels(pull).includes(PENDING_LABEL) ||
		labels(pull).includes(TAGGED_LABEL)
	);
}
function exactPulls(pulls, candidateSha) {
	const releasePulls = pulls.filter(shaped);
	return {
		exact: releasePulls.filter(
			(pull) =>
				pull?.state === 'closed' &&
				typeof pull?.merged_at === 'string' &&
				pull?.merge_commit_sha === candidateSha
		),
		stale: releasePulls.filter(
			(pull) =>
				pull?.state === 'closed' &&
				typeof pull?.merged_at === 'string' &&
				pull?.merge_commit_sha !== candidateSha
...
export async function hydrateExactReleasePullTree(
	repository,
	candidateSha,
	pulls,
	request = api
...
async function api(path, options = {}) {
	...
	const response = await fetch(`https://api.github.com${path}`, {
		...
		headers: {
			...
			'User-Agent': 'ran-wp-release-updater-exact-publisher',
...
async function associatedPulls(repository, sha) {
	const pulls = [];
...
export async function runPublisher(root = process.cwd()) {
	...
	const unreleased =
		manifestVersion(candidateContents.manifest, 'candidate', true) ===
			'0.0.0' &&
		runtimeCopy(candidateContents.runtimeCopy).package_version === '0.0.0';
	const identity = unreleased
		? {
				candidateSha: sha,
				version: '0.0.0',
				tag: 'v0.0.0',
				notes: 'unreleased bootstrap',
				packageRevision: runtimeCopy(candidateContents.runtimeCopy)
					.package_revision,
			}
	...
	const hydratedPulls = await hydrateExactReleasePullTree(
		repository,
		sha,
		pulls
	);
	const changedPaths = git(root, ['diff', '--name-only', `${sha}^`, sha])
		.split('\n')
		.filter(Boolean)
		.sort();
	const parents = git(root, ['show', '-s', '--format=%P', sha])
		.split(' ')
		.filter(Boolean)
		.map((value) => ({ sha: value }));
	let input = {
		...
		commit: {
			...
			tree: { sha: git(root, ['show', '-s', '--format=%T', sha]) },
		},
		repository,
	...
	const original = input.pulls.find(
		(pull) => pull.number === result.pullNumber
	);
	await reconcileLabels(repository, result.pullNumber, labels(original));
	const finalPull = (
		await api(`/repos/${repository}/pulls/${result.pullNumber}`)
	).data;
	const finalWithTree = (
		await hydrateExactReleasePullTree(repository, sha, [finalPull])
	)[0];
	decidePublication({
		...input,
		pulls: [finalWithTree],
