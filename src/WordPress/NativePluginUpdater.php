...
final class NativePluginUpdater {
	...
	public function filterPreDownload( mixed $reply, string $package, mixed $upgrader, array $hookExtra ): mixed {
		unset( $upgrader ); if ( ! $this->matchesOperation( $hookExtra ) ) return $reply;
	...
	private static function validConfiguration( array $configuration, BindingRecord $binding ): bool {
		...
		return $facts['canonical_update_uri'] === CanonicalUpdateUri::canonicalizeBoundaries( array( 'archive_preflight' => $facts['canonical_update_uri'], 'configuration' => $configuration['update_uri'], 'offer_or_cache' => $configuration['headers']['UpdateURI'], 'staged_package' => $facts['canonical_update_uri'] ) );
	}

	private function matchesRuntimeUri( string $runtimeUri ): bool { return $this->updateUri === CanonicalUpdateUri::canonicalizeBoundaries( array( 'archive_preflight' => $this->binding->toArray()['canonical_update_uri'], 'configuration' => $this->updateUri, 'offer_or_cache' => $runtimeUri, 'staged_package' => $this->headers['UpdateURI'] ) ); }
	...
	private function matchesStagedMetadata( string $source ): bool { $source = rtrim( $source, '/\\' ); $directory = @lstat( $source ); if ( ! is_array( $directory ) || 0040000 !== ( $directory['mode'] & 0170000 ) || ! hash_equals( $this->packageRoot(), basename( $source ) ) ) return false; $header = 'plugin' === $this->targetType ? basename( $this->installedIdentity ) : 'style.css'; $path = $source . DIRECTORY_SEPARATOR . $header; $file = @lstat( $path ); if ( ! is_array( $file ) || 0100000 !== ( $file['mode'] & 0170000 ) ) return false; $contents = @file_get_contents( $path, false, null, 0, 8192 ); if ( ! is_string( $contents ) ) return false; $name = self::headerValue( $contents, 'theme' === $this->targetType ? 'Theme Name' : 'Plugin Name' ); $version = self::headerValue( $contents, 'Version' ); $uri = self::headerValue( $contents, 'Update URI' ); $facts = $this->descriptor->toArray(); return is_string( $name ) && hash_equals( $this->headers['Name'], $name ) && is_string( $version ) && is_string( $uri ) && 0 === ReleaseVersion::compare( $version, $facts['version'] ) && $this->matchesRuntimeUri( $uri ); }
	...
	private function discover( string $installed ): ?IdentityDescriptor {
		try { $listed = $this->adapter->listReleases(); $candidates = $listed['candidates'] ?? null; } catch ( \Throwable ) { return null; }
		if ( ! is_array( $candidates ) || count( $candidates ) > 8 ) return null;
		foreach ( $candidates as $candidate ) {
			if ( ! is_array( $candidate ) || ! is_string( $candidate['release_identity'] ?? null ) || ! is_string( $candidate['tag'] ?? null ) || ! is_string( $candidate['version'] ?? null ) ) continue;
			if ( ReleaseVersion::RELATIONSHIP_NEWER !== ReleaseVersion::relationship( $candidate['version'], $installed ) ) continue;
			try { $descriptor = $this->adapter->inspect( $candidate['release_identity'], $candidate['tag'] ); BindingRecord::assertDescriptorBinding( $descriptor, $this->binding ); } catch ( \Throwable ) { continue; }
			$facts = $descriptor->toArray();
			if ( ! hash_equals( $candidate['release_identity'], $facts['release_identity'] ) || ! hash_equals( $candidate['tag'], $facts['tag'] ) || 0 !== ReleaseVersion::compare( $candidate['version'], $facts['version'] ) ) continue;
			try { $artifact = $this->adapter->acquire( $descriptor ); $valid = $artifact->inspect( fn( string $path ) => $this->validator->validate( $descriptor, $this->archivePolicy, $path ) ); } catch ( \Throwable ) { continue; }
