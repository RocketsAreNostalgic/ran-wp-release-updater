...
final class PackageIdentityValidator {
	...
	public function __construct( ?\Closure $afterOpen = null ) {
		$this->afterOpen     = $afterOpen;
		$this->receiptProofs = new WeakMap(); }
	...
	public function validate( IdentityDescriptor $descriptor, array $policy, string $archivePath ): ValidatedPackage {
		...
		try {
			...
			if ( ! $this->matchesArchiveIdentity( $archivePath, $descriptorFacts, $identity ) ) {
				return ValidatedPackage::blocked( 'archive_file_identity_mismatch' );
			}
			...
			if ( null === $version || 0 !== ReleaseVersion::compare( $version, $descriptorFacts['version'] ) ) {
				return ValidatedPackage::blocked( 'archive_version_mismatch' );
			}
			if ( null === $requiresPhp || ( is_string( $requiresPhp ) && ! self::meetsRequirement( $policy['php_runtime_version'], $requiresPhp ) ) ) {
				return ValidatedPackage::blocked( 'archive_php_requirement_incompatible' );
			}
			if ( null === $requiresWordPress || ( is_string( $requiresWordPress ) && ! self::meetsRequirement( $policy['wordpress_runtime_version'], $requiresWordPress ) ) ) {
			...
			return $this->ready(
				array(
					...
					'manifest_expanded_bytes' => $manifestBytes,
	...
	public function consumeReceiptProof( ValidatedPackage $package, IdentityDescriptor $descriptor ): array {
		if ( ! isset( $this->receiptProofs[ $package ] ) ) {
	...
	private function ready( array $snapshot ): ValidatedPackage {
		...
		$this->receiptProofs[ $package ] = array(
	...
	private static function readHeader( \ZipArchive $zip, string $name ): ?string {
	...
	private static function entryIdentity( \ZipArchive $zip, string $name, int $expectedSize ): ?array {
		$stream = $zip->getStream( $name );
		if ( ! is_resource( $stream ) ) {
			return null;
		...
		while ( ! feof( $stream ) ) {
			...
			if ( ! is_string( $chunk ) || ( '' === $chunk && ! feof( $stream ) ) || strlen( $chunk ) > $expectedSize - $size ) {
				$valid = false;
				break;
			} $size += strlen( $chunk );
			hash_update( $context, $chunk );
		} fclose( $stream );
		return $valid && $size === $expectedSize ? array(
	...
	private static function meetsRequirement( string $runtimeVersion, string $requiredVersion ): bool {
		$comparison = ReleaseVersion::compare( $runtimeVersion, $requiredVersion );
